...
export class GeminiService implements LLMService {
    ...
    async summarize(prompt: string): Promise<string> {
        try {
            ...
            const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/${this.settings.modelName}:generateContent?key=${this.settings.geminiApiKey}`;
            
            // Check if this is a multimodal request with a video URL
            const isMultimodalRequest = 
                (this.settings.modelName.includes('pro') || 
                 this.settings.modelName.includes('vision')) && 
            ...
            if (isMultimodalRequest) {
                ...
                if (videoUrl) {
                    ...
                    requestBody = {
                        ...
                        generationConfig: {
                            temperature: this.settings.temperatureVal,
